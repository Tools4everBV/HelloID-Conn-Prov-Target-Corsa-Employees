#################################################
# HelloID-Conn-Prov-Target-Corsa-Employees-Update
# Update csv row
# PowerShell V2
#################################################
    ...
    switch ($action) {
        "Update" {
            ...
            if (-Not($actionContext.DryRun -eq $true)) {
                $null = $updatedCsvContent | Foreach-Object { $_ } | Select-Object "Code_Pers_Lid",   
                            "Naam_Pers_Lid",       
                            "Voorletters",        
                            "Tussenvoegsels", 
                            "Titels",    
                            ...
                            "Afdeling",            
                            "Organisatie",  
                            "Bevoegden"     | ConvertTo-Csv -Delimiter $exportCsvSplatParams.Delimiter | Foreach-Object {$_ -replace '"',''} |  Set-Content -path $actionContext.Configuration.CsvPath -Encoding $actionContext.Configuration.Encoding
            ...
            if (-Not($actionContext.DryRun -eq $true)) {
                if (Test-Path $actionContext.Configuration.CsvPath) {
                    $null = $updatedCsvContent | Foreach-Object { $_ } | Select-Object "Code_Pers_Lid",   
                            ...
                            "Organisatie",  
                            "Bevoegden"     |
                                        ConvertTo-Csv -Delimiter $exportCsvSplatParams.Delimiter | Foreach-Object {$_ -replace '"',''} |  Set-Content -path $actionContext.Configuration.CsvPath -Encoding $actionContext.Configuration.Encoding
                }
                else {
                    $account | Select-Object "Code_Pers_Lid",   
                            "Naam_Pers_Lid",       
                            "Voorletters",        
                            "Tussenvoegsels", 
                            "Titels",    
